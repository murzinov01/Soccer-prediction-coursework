...
class DataPackager:
    ...
    def convert_match_data(self, match: dict, data: dict) -> dict:
        ...
        for end in endings:
            ...
            d_var = self.AL.summarise_statistic(match_id, use_prev_season=False if self.AL.number_of_tour(match_id)[
                                                                                       "TeamHome"] >= self.MIN_GAMES else True, period=period)
            for key in d_var["TeamHome"]["HomeStats"].keys():
                ...
                data[key + "Home" + end] = stats

            d_var = self.AL.summarise_statistic(match_id, use_prev_season=False if self.AL.number_of_tour(match_id)[
                                                                                       "TeamAway"] >= self.MIN_GAMES else True, period=period)
